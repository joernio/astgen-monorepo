...
const parser = path.join(import.meta.dir, "..", "src", "parse-abap.ts")

// process.execPath is the bun binary running the tests, so the parser is
// exercised the same way it runs in production (bun executing TS directly).
function runParser(args: string[]): SpawnSyncReturns<string> {
    return spawnSync(process.execPath, [parser, ...args], {encoding: "utf8"})
}
...
function runFixture(code: string, filename: string, assertFn: (fixture: Fixture) => void): void {
    ...
    const result = runParser([srcDir, outDir])
    try {
...
function readJsonOutput(outDir: string, abapFilename: string): AstJson {
...
`
    ...
    it("should accept a single .abap file as input (not just a directory)", () => {
        ...
        try {
            const result = runParser([inputFile, outDir])
            expect(result.status).toBe(0)
    ...
    it("should fail (non-zero exit) when the input path does not exist", () => {
        ...
        const outDir = path.join(tmpDir, "out")
        try {
            const result = runParser([missing, outDir])
            expect(result.status).not.toBe(0)
    ...
    it("should not crash on an empty .abap file", () => {
        runFixture("", "z_empty.prog.abap", ({result}) => {
            expect(result.status).toBe(0)
...
`
    ...
    it("should not crash when given a filename that does not map to an ABAP object", () => {
        ...
        runFixture(code, "random_name.abap", ({result}) => {
            expect(result.status).toBe(0)
